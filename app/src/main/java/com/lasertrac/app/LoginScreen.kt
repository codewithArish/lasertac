...
import com.lasertrac.app.network.models.LoginRequest // Corrected import path
...
fun LoginScreen(onLoginSuccess: () -> Unit, onNavigateToCreateAccount: () -> Unit) {
    ...
    val retrofitInstance = remember { RetrofitInstance(context) }
            ...
            if (isLoading) {
                ...
                Button(
                    onClick = {
                        ...
                        scope.launch {
                            try {
                                // Use the instantiated retrofitInstance
                                val response = retrofitInstance.api.login(LoginRequest(email, password))

                                if (response.isSuccessful) {
                                    val authResponse = response.body()
                                    if (authResponse?.status == "success") {
                                        sessionManager.saveLoginSession() // Save session
